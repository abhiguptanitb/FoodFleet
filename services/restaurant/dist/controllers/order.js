...
export const getNearbyReadyOrdersForRider = TryCatch(async (req, res) => {
    if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
        return res.status(403).json({
            message: "Forbidden",
        });
    }
    const latitudeParam = Array.isArray(req.query.latitude)
        ? req.query.latitude[0]
        : req.query.latitude;
    const longitudeParam = Array.isArray(req.query.longitude)
        ? req.query.longitude[0]
        : req.query.longitude;
    const radiusParam = Array.isArray(req.query.radius)
        ? req.query.radius[0]
        : req.query.radius;
    const latitude = Number(latitudeParam);
    const longitude = Number(longitudeParam);
    const radius = Number(radiusParam || 500);
    ...
    const restaurantIds = nearbyRestaurants.map((restaurant) => restaurant._id.toString());
    const orders = await Order.find({
...
export const getRiderDeliveredStats = TryCatch(async (req, res) => {
    if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
        return res.status(403).json({
            message: "Forbidden",
        });
...
export const getRestaurantSalesStats = TryCatch(async (req, res) => {
    ...
    try {
        ...
        const topItemArray = Array.from(itemSalesMap.entries()).sort((a, b) => b[1] - a[1]);
...
export const getRestaurantPerformance = TryCatch(async (req, res) => {
    const user = req.user;
    const restaurantId = typeof req.params.restaurantId === "string"
        ? req.params.restaurantId
        : "";
    ...
    const platformFees = orders.reduce((sum, order) => sum + (order.platfromFee || 0), 0);
    const riderPayouts = orders.reduce((sum, order) => sum + getOrderRiderEarning(order), 0);
    res.json({
        range,
        ...
        payout: {
            ...
            estimatedSellerPayout: Number((grossRevenue - platformFees - riderPayouts).toFixed(2)),
