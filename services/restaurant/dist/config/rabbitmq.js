...
export const connectRabbitMQ = async () => {
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
        try {
            ...
            lastError = error;
            if (attempt < 5) {
                console.warn(`RabbitMQ connection failed for restaurant service. Retrying (${attempt}/5)...`);
                await delay(2000);
            }
        }
    }
    console.error(`Last RabbitMQ error: ${getErrorMessage(lastError)}`);
    throw new Error("Restaurant service could not connect to RabbitMQ. Make sure RabbitMQ is running and RABBITMQ_URL points to it.");
};
