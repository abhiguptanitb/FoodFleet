...
export const connectRabbitMQ = async () => {
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    try {
      ...
      if (attempt < 5) {
        console.warn(
          `RabbitMQ connection failed for restaurant service. Retrying (${attempt}/5)...`
        );
        await delay(2000);
      }
    }
  ...
  throw new Error(
    "Restaurant service could not connect to RabbitMQ. Make sure RabbitMQ is running and RABBITMQ_URL points to it."
  );
};
