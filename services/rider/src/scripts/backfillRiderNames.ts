...
const orderRiderDocument = (rider: RiderDocument, riderName: string) => {
...
const orderOrderDocument = (order: OrderDocument) => {
  const ordered: Record<string, unknown> = {
    _id: order._id,
    userId: order.userId,
...
const run = async () => {
  ...
  const riders = db.collection<RiderDocument>("riders");
  ...
  for await (const rider of riderCursor) {
    ...
    const riderResult = await riders.replaceOne(
      { _id: rider._id },
      orderedRider
    );

    const orderFilter =
      isImageUrl(rider.riderName) || !rider.riderName?.trim()
        ? {
  ...
  console.log(
    `Backfill complete. Scanned ${scanned} riders, updated ${ridersUpdated} riders, updated ${ordersUpdated} order rider fields, reordered ${ordersReordered} orders, skipped ${skipped}.`
  );
