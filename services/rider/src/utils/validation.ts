...
const aadharNumber = Joi.string()
  .trim()
  .custom((value: string) => value.replace(/\s/g, ""))
  .pattern(/^\d{12}$/)
  .required()
  .messages({
...
const drivingLicenseNumber = Joi.string()
  ...
  .messages({
    ...
    "string.pattern.base":
      "Driving licence number must start with 2 letters, 2 digits, and contain only letters/numbers",
...
const riderProfileSchema = (requireLocation = false) =>
  Joi.object<RiderProfileInput>({
    phoneNumber,
    aadharNumber,
...
const locationSchema = Joi.object<{ latitude: number; longitude: number }>({
...
export const validateRiderProfileInput = (
  body: Record<string, unknown>,
  options: { requireLocation?: boolean } = {}
): ValidationResult<RiderProfileInput> => {
  return validateWithSchema(
    riderProfileSchema(Boolean(options.requireLocation)),
    body
  );
};
