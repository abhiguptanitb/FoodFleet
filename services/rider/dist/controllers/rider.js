...
export const updateRiderProfile = TryCatch(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({
            message: "Unauthorized",
...
export const fetchNearbyAvailableOrders = TryCatch(async (req, res) => {
    const riderUserId = req.user?._id;
    if (!riderUserId) {
        return res.status(400).json({
            message: "Please Login",
    ...
    const { data } = await axios.get(`${process.env.RESTAURANT_SERVICE}/api/order/nearby-ready/rider`, {
        params: {
            latitude,
            longitude,
            radius: 500,
...
export const fetchRiderDashboardStats = TryCatch(async (req, res) => {
    ...
    const { data } = await axios.get(`${process.env.RESTAURANT_SERVICE}/api/order/stats/rider`, {
        params: {
            riderId: rider._id.toString(),
            range,
        },
...
export const fetchRiderOrderHistory = TryCatch(async (req, res) => {
    const userId = req.user?._id;
    if (!userId) {
        return res.status(401).json({
            message: "Please Login",
    ...
    const { data } = await axios.get(`${process.env.RESTAURANT_SERVICE}/api/order/history/rider`, {
        params: {
            riderId: rider._id.toString(),
            range,
            limit: limitParam || 20,
